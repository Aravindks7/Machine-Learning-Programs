...
import math
def entropy (probs):
  return sun([-prob*math.log(prob, 2) for prob in probs])
...
def id3(df,target, attribute_name, default_class None):
  ...
  if len(cnt)==1:
    ...
    gains = [info gain (df, attr, target) for attr in attribute_name] 
    index_max = gains.index(max(gains))
    best_attr = attribute_name[index_max]
    tree = {best_attr: { } }
    remaining attr = [x for x in attribute_name if xl-best_attr]
    for attr_val, data_subset in df.groupby(best_attr):
      subtree = id3 (data_subset, target, remaining attr,default_class) 
